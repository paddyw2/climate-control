...
/**
 * <h1>Moisture Settings Pop Up</h1>
 *
 * A JDialog that allows the user to update
 * their moisture simulation settings.
 *
 * @author  Patrick Withams
 * @version 1.0
 * @since   2016-04-08
 */
public class MoistureSettings extends JDialog {
     ...
     * @param parent
     */
    public MoistureSettings(JFrame parent)
    {
        super(parent, true);
        this.setTitle("Moisture Settings");
        ...
        fieldOne.setToolTipText("Enter starting moisture (%)");
        fieldTwo = new JTextField(4);
        fieldTwo.setToolTipText("Enter lower acceptable moisture (%)");
        fieldTwo2 = new JTextField(4);
        fieldTwo2.setToolTipText("Enter upper acceptable moisture (%)");
        fieldThree = new JTextField(10);
        fieldThree.setToolTipText("Enter moisture rate, in % per second i.e. 0.0167 = 1% per minute");

        fieldFour = new JTextField(10);
        fieldFour.setToolTipText("Enter sensor refresh rate in seconds");
        fieldFive = new JTextField(10);
        fieldFive.setToolTipText("Enter the external moisture effect, in % per second i.e. 0.0167 = 1% per minute");

        Border padding = BorderFactory.createEmptyBorder(5,0,0,0);

        fieldOneText = new JLabel("Starting Moisture:");
        fieldOneText.setBorder(padding);
        fieldTwoText = new JLabel("Desired Range:");
        fieldTwoText.setBorder(padding);
        fieldThreeText = new JLabel("Sprinkler Rate:");
        ...
        closeButton.setName("moistureSettings");
     ...
     */
    public double getOrigMoisture() throws NumberFormatException
    {
        return Double.parseDouble(fieldOne.getText());
    }

    public double getMoistureLower() throws NumberFormatException
    {
        return Double.parseDouble(fieldTwo.getText());
    }

    public double getMoistureUpper() throws NumberFormatException
    {
        return Double.parseDouble(fieldTwo2.getText());
    }

    public double getMoistureRate() throws NumberFormatException
    {
        return Double.parseDouble(fieldThree.getText());
    }

    public double getMoistureSample() throws NumberFormatException
    {
        return Double.parseDouble(fieldFour.getText());
    }

    public double getMoistureExternal() throws NumberFormatException
    {
